        ...
        implements ICardActionHandler, IInputWatcher
{
    ...
    private static final String TAG = "GlipAdaptiveCards";
    private static final String CARD_INTENT_DATA = "JsonData";
    private static String IS_CARD = "isCard";
    private final String defaultCard = "v1.0/Scenarios/ActivityUpdate.json";
    private final String defaultHostConfig = "HostConfig/sample.json";
    ...
    private String m_customHostConfigData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        m_customHostConfigData = getIntent().getStringExtra(CARD_INTENT_DATA);
        ...
        try {
            String filename = defaultCard.substring(defaultCard.lastIndexOf('/'));
            loadAdaptiveCard(filename, readStream(getAssets().open(defaultCard)));
        } catch (IOException e) {
            e.printStackTrace();
        }

        if (!TextUtils.isEmpty(m_customHostConfigData)) {
            loadHostConfig("Glip custom config", m_customHostConfigData);
        } else {
            String filename = defaultHostConfig.substring(defaultHostConfig.lastIndexOf('/'));
            try {
                loadHostConfig(filename, readStream(getAssets().open(defaultHostConfig)));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    {
        Log.d(TAG, "loadHostConfig : " + hostConfig);
